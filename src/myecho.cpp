...
/******************************************************************************/
void
PrintIPAddress(const char *pcInformationString, IPAddress &cAddr)
{
    printf("%s\n", pcInformationString);
    printf(" - Name    : %s\n", (const char *) cAddr);
...
/******************************************************************************/
bool
SetFlag(char *pcArg, const char *pcTrueString, const char *pcFalseString, bool &bFlag, bool bDefault)
{
    bFlag = false;
...
main(int argc, char **argv)
{
    ...
    {
        iCurrentArg = 1;
        if (iCurrentArg == argc) throw iCurrentArg;

        if (SetFlag(argv[iCurrentArg], "serv", "client", bServer, false))
        {
            iCurrentArg++;
            if (iCurrentArg == argc) throw iCurrentArg;
        }
        if (SetFlag(argv[iCurrentArg], "ip6", "ip4", bUseIPv6, false))
        {
            iCurrentArg++;
            if (iCurrentArg == argc) throw iCurrentArg;
        }
        if (SetFlag(argv[iCurrentArg], "tcp", "udp", bUseTCP, true))
        {
            iCurrentArg++;
            if (iCurrentArg == argc) throw iCurrentArg;
        }
        ...
        if (!bServer)
        {
            try
            {
                cEchoServerAddress.SetHostName(argv[iCurrentArg], bUseIPv6);
                iCurrentArg++;
                if (iCurrentArg == argc) throw iCurrentArg;
            }
        ...
        char *pcEndNumber;
        iServerPortNumber = strtol((const char *)argv[iCurrentArg], &pcEndNumber, 10);
    ...
    catch (int &iBadArg)
    {
        ...
        printf("./%s ", argv[0]);
        for (iCurrentArg = 1; iCurrentArg < argc; iCurrentArg++)
        {
            printf((iCurrentArg == iBadArg)?"(%s) ":"%s ", argv[iCurrentArg]);
        }
        printf("\n\n");
